...
namespace Solid.TrieMap
{
	internal abstract class MapNode<TKey, TValue>
	{
		...
		public abstract bool TryContains(HashedKey<TKey> tryKey, ref Result result1);

		public abstract MapNode<TKey, TValue> TryDrop(HashedKey<TKey> hashedKey, ref Result result1);

		/// <summary>
		///   Tries to get a Leaf from the map using a hashed key.
		/// </summary>
		/// <param name="tryKey"> The key. </param>
		/// <param name="result"> An output parameter that returns the status code of the operation. </param>
		/// <returns> The Leaf, or null if no key was found. </returns>
		public abstract TValue TryGet(HashedKey<TKey> tryKey, ref Result result);
		                                   ...
		                                   List<KeyValuePair<TKey, TValue>> collisions)
		{
			...
			switch (code)
			{
				...
				case NodeType.Leaf << 3 | NodeType.Parent:
					...
					output = other.TrySet(oneAsLeaf.MyKey, oneAsLeaf.MyValue, WriteBehavior.OnlyCreate, ref outcome);
					switch (outcome)
					{
				...
				case NodeType.Parent << 3 | NodeType.Leaf:
					...
					output = one.TrySet(twoAsLeaf.MyKey, twoAsLeaf.MyValue, WriteBehavior.Any, ref outcome);
					switch (outcome)
					{
						case Result.HashCollision:
