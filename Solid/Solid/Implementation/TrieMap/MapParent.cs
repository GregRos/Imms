...
namespace Solid.TrieMap
{
	internal sealed class MapParent<TKey, TValue> : MapNode<TKey, TValue>
	{
			...
			: base(height, 2, NodeType.Parent)
		{
			...
			int localKey1 = ((key1.Hash & myBlock) >> myCount);
			int localKey2 = ((key2.Hash & myBlock) >> myCount);
		...
		{
			int localKey = (((tryKey.Hash & myBlock) >> myCount));
			if ((Population & (1u << localKey)) == 0u)
			{
				result = Result.Success;
				return false;
			}
			uint population = Population;
			unchecked
			{
			...
			return Arr[population].TryContains(tryKey, out result);
		}
		...
		public override MapNode<TKey, TValue> TryDrop(HashedKey<TKey> tryKey, out Result result)
		{
			...
			MapNode<TKey, TValue> newNode = Arr[population].TryDrop(tryKey, out result);
			...
			switch (result)
			{
				...
				case Result.TurnedEmpty:
					...
					uint newPop = (Population & ~((1u << localKey) - 1));
					result = Result.Success;
		...
		public override TValue TryGet(HashedKey<TKey> tryKey, out Result result)
		{
			...
			return Arr[population].TryGet(tryKey, out result);
		}
		                                             ...
		                                             out Result result)
		{
			...
			if ((Population & (1u << localKey)) == 0u)
			{
				...
				uint newPop = (Population | ((1u << localKey)));
				result = Result.Success;
				return new MapParent<TKey, TValue>(Height, Count + 1, newArr, newPop);
			}
			uint population1 = Population;
			unchecked
			{
			...
			MapNode<TKey, TValue> oldNode = Arr[population1];
			newNode = oldNode.TrySet(tryKey, tryValue, behave, out result);
			switch (result)
			{
				case Result.Success:
