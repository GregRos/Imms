...
namespace Funq {
	...
	static class Convertion {
		...
		internal static FunqMap<TKey, TValue> WrapMap<TKey, TValue>(this HashedAvlTree<TKey, TValue>.Node root,
			IEqualityComparer<TKey> equality) {
			return new FunqMap<TKey, TValue>(root, equality);
		}

		internal static FunqOrderedMap<TKey, TValue> WrapMap<TKey, TValue>(this OrderedAvlTree<TKey, TValue>.Node root,
		...
		public static FunqVector<T> Wrap<T>(this TrieVector<T>.Node inner) {
