...
namespace Funq.Collections {
	...
	public partial class FunqOrderedMap<TKey, TValue> : AbstractMap<TKey, TValue, FunqOrderedMap<TKey, TValue>> {
		FunqOrderedMap<TKey2, TValue2> GetPrototype<TKey2, TValue2>(IComparer<TKey2> ph) {
		...
		/// <returns></returns>
		public FunqOrderedMap<TRKey, TRValue> Select<TRKey, TRValue>(
			Func<KeyValuePair<TKey, TValue>, KeyValuePair<TRKey, TRValue>> selector, IComparer<TRKey> handler) {
		...
		/// <returns></returns>
		public FunqOrderedMap<TRKey, TRValue> Select<TRKey, TRValue>(
			Func<KeyValuePair<TKey, TValue>, Option<KeyValuePair<TRKey, TRValue>>> selector, IComparer<TRKey> handler) {
			if (selector == null) throw Errors.Is_null;
		...
		/// <returns></returns>
		public FunqOrderedMap<TRKey, TRValue> Select<TRKey, TRValue>(
			Func<TKey, TValue, KeyValuePair<TRKey, TRValue>> selector, IComparer<TRKey> handler) {
			if (selector == null) throw Errors.Is_null;
		...
		/// <returns></returns>
		public FunqOrderedMap<TKey, TRValue> Join<TValue2, TRValue>(IAnyMapLike<TKey, TValue2> other,
			Func<TKey, TValue, TValue2, TRValue> collision) {
		...
		/// <param name="handler">A new equality or comparison handler for constructing the resulting map.</param>
		/// <returns></returns>
		public FunqOrderedMap<TRKey, TRValue> SelectMany<TRKey, TRValue, TProject>(
			Func<KeyValuePair<TKey, TValue>, IEnumerable<TProject>> selector,
			Func<KeyValuePair<TKey, TValue>, IEnumerable<TProject>, KeyValuePair<TRKey, TRValue>> rSelector,
			IComparer<TRKey> handler) {
			if (selector == null) throw Errors.Is_null;
