...
namespace Funq.Collections
{
	public partial class FunqOrderedSet<T>
	{
		internal class Builder : SetBuilder<T>
		{
			...
			{
				_inner = _inner.AvlAdd(_comparer.WrapKey(item), true, Lineage.Mutable());
			}

			public override bool Contains(T item)
			{
				return _inner.Find(_comparer.WrapKey(item)).IsSome;
			}

			public override void Remove(T item)
			{
				_inner = _inner.AvlRemove(_comparer.WrapKey(item), Lineage.Mutable());
			}
		}
