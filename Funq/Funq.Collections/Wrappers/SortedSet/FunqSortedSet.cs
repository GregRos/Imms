...
namespace Funq.Collections
{
	public partial class FunqOrderedSet<T> : Trait_SetLike<T, FunqOrderedSet<T>>
	{
		...
		{
			//if (this.Contains(item)) throw Funq.Errors.Key_exists;
			return _root.AvlAdd(_comparer.WrapKey(item), true, Lineage.Mutable()).WrapSet(_comparer);
		}


		public FunqOrderedSet<T> Drop(T item)
		{
			var removed = _root.AvlRemove(_comparer.WrapKey(item), Lineage.Mutable()) ?? _root;
		...
		public FunqOrderedSet<T> AddMany(IEnumerable<T> items)
		{
			...
			{
				newRoot = newRoot.AvlAdd(_comparer.WrapKey(item), true, lineage);
			}
		...
		public FunqOrderedSet<T> DropMany(IEnumerable<T> items)
		{
			...
			foreach (var item in items)
			{
				newRoot = newRoot.AvlRemove(_comparer.WrapKey(item), lineage) ?? newRoot;
			}
			return newRoot.WrapSet(_comparer);
		}
		...
		{
			return _root.Find(_comparer.WrapKey(item)).IsSome;
		}
	}
}
