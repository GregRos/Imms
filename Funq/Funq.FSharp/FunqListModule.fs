...
module Funq.FSharp.FunqList
open Funq.Collections
let empty<'t>                      = FunqList<'t>.Empty

let inline private asArr o         = o :_ FunqList
///Generic "specialized pipe" operator for reslolving type inference.
let inline (=>) (x : _ FunqList) f = f x
 
let ofItem v                       = FunqList<'t>.Empty <+ v

let ofSeq vs                       = FunqList<'t>.Empty <++ vs
...
let addFirst v arr                 = arr => GenSeq.addFirst v
let addFirstRange vs arr           = arr => GenSeq.addFirstRange vs
let addFirstList vs arr            = arr => GenSeq.addFirstList vs
let addLastList vs arr             = arr => GenSeq.addLastList vs