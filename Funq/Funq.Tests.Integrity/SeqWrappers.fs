...
type FunqListWrapper<'v>(inner : FunqList<'v>)= 
    ...
    let wrap x = FunqListWrapper(x) :> SeqWrapper<_>
    ...
    override x.Remove(i) = FunqListWrapper(inner.RemoveAt(i)) :>_
    ...
    override x.Range(st,en,step) = FunqList.ofSeq [st..(step.Or 1)..en] |> wrap
    ...
    override x.Inner = inner:>_
    override x.All f = inner |> FunqList.forAll f
    override x.Any f = inner |> FunqList.exists f
    override x.Count f = inner |> FunqList.count f
    override x.Pick f = inner |> FunqList.pick f
    override x.Find f = inner |> FunqList.find f
    override x.Single() = inner.Single()
    override x.Fold initial f = inner.Aggregate(initial, toFunc2 f)
    override x.Reduce f = inner |> FunqList.reduce f
    override x.FindIndex f = inner.FindIndex f |> fromOption
    override x.FindLast (f: _ -> _) = inner |> FunqList.findLast f
    override x.ReduceBack f = inner |> FunqList.reduceBack f
    override x.FoldBack initial f = inner |> FunqList.foldBack initial f
    override x.SkipWhile f = inner |> FunqList.skipWhile f |> wrap
    override x.TakeWhile f = inner |> FunqList.takeWhile f |> wrap
    override x.Reverse() = inner.Reverse() |> wrap
    static member FromSeq s= FunqListWrapper(FunqList.ofSeq s) :> SeqWrapper<_>

type FunqVectorWrapper<'v>(inner : FunqVector<'v>)= 
    inherit SeqWrapper<'v>("FunqVector")
    let wrap x = FunqVectorWrapper x :> SeqWrapper<_>
    override x.GetEnumerator() = (inner :> 'v seq).GetEnumerator()
    ...
    override x.Single() = inner.Single()
    override x.FindLastIndex f = inner.FindLastIndex (f |> toFunc1) |> fromOption
    override x.Range(st,en,?step)= FunqVector.ofSeq [st..(step.Or 1)..en] |> wrap
    ...
    override x.All (f : 'v -> bool) = inner |> FunqVector.forAll f
    override x.Any f = inner |> FunqVector.exists f
    override x.Count f = inner |> FunqVector.count f
    override x.Pick f = inner |> FunqVector.pick f
    override x.Find f = inner |> FunqVector.find f
    override x.Fold initial f = inner.Aggregate(initial, toFunc2 f)
    override x.Reduce f = inner |> FunqVector.reduce f
    override x.FindIndex f = inner |> FunqVector.findIndex f
    override x.FindLast (f: _ -> _) = inner |> FunqVector.findLast f
    
    override x.ReduceBack f = inner |> FunqVector.reduceBack f
    override x.FoldBack initial f = inner |> FunqVector.foldBack initial f
    override x.SkipWhile f = inner |> FunqVector.skipWhile f |> wrap
    override x.TakeWhile f = inner |> FunqVector.takeWhile f |> wrap
    override x.Reverse() = inner.Reverse() |> wrap
    override x.Inner = inner:>_
    
    static member FromSeq s = FunqVectorWrapper(FunqVector.ofSeq (s))  :> SeqWrapper<_>
