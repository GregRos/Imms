...
type FunqOrderedMapWrapper<'k when 'k : comparison>(Inner : FunqOrderedMap<'k, 'k>) =
    inherit MapWrapper<'k>("FunqOrderedMap")
    static let wrap x = FunqOrderedMapWrapper<'k>(x) :> MapWrapper<'k>
    let unwrap (y : Kvp<'k,'k> seq) = 
        match y with
        | :? FunqOrderedMapWrapper<'k> as map -> map.Inner :> _ seq
        | _ -> y
    let unwrapSet (y : 'k seq) =
        match y with
        | :? FunqOrderedSetWrapper<'k> as set -> set.Inner :> _ seq
        | _ -> y
    ...
    override x.Empty = FunqOrderedMap.emptyWith (Cmp.Default) |> wrap
    override x.EmptySet = FunqOrderedSetWrapper<'k>.FromSeq []
    ...
    override x.Pick f = Inner |> FunqOrderedMap.pick f
    override x.Find f = Inner.Find f |> fromOption
    override x.Fold initial f = Inner |> FunqOrderedMap.fold initial f
    override x.Reduce f = Inner |> FunqOrderedMap.reduce f
    static member FromSeq s = 
        FunqOrderedMapWrapper(FunqOrderedMap.ofSeqWith Cmp.Default s) :> MapWrapper<_>

type FunqMapWrapper<'k when 'k : comparison>(Inner : FunqMap<'k,'k>) =
    inherit MapWrapper<'k>("FunqMap")
    static let wrap x = FunqMapWrapper<'k>(x) :> MapWrapper<'k>
    static let unwrap (xs : Kvp<'k,'k> seq) =
        match xs with
        | :? FunqMapWrapper<'k> as wrapped -> wrapped.Inner :> _ seq
        | _ -> xs
    static let unwrapSet (xs : 'k seq) = 
        match xs with
        | :? FunqSetWrapper<'k> as wrapped -> wrapped.Inner :> _ seq
        | _ -> xs
    ...
    override x.EmptySet = FunqSetWrapper<'k>.FromSeq []
    override x.MapEquals other = Inner.MapEquals(other)
    override x.SetRange vs = Inner.SetRange (vs |> unwrap) |> wrap
    override x.AddRange vs = Inner.AddRange (vs |> unwrap) |> wrap
    override x.SelfTest() = 
    ...
    override x.Any f = Inner.Any f
    override x.Count f = Inner.Count f
    override x.Pick f = Inner |> FunqMap.pick f
    override x.Find f = Inner.Find f |> fromOption
    override x.Fold initial f = Inner.Aggregate(initial, toFunc2 f)
    override x.Reduce f = Inner |> FunqMap.reduce f
    static member FromSeq s = FunqMapWrapper(FunqMap.ofSeq'(s))  :> MapWrapper<_>
