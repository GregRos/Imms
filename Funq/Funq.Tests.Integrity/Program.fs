...
let main argv = 
    ...
    let tests = 
        ...
        Test.update; Test.insert; Test.insert_remove_update] |> List.cross_apply1 [500]// |> List.filter (fun test -> test?Name |> String.contains "remove")

    let tests_seq = [Test.slices; Test.take; Test.get_index; Test.concat] |> List.cross_apply1 [500]
    let targets = [Target.FunqArray; Target.FunqList] |> List.cross_apply1 [500]
    let reference = Target.Reference 500
    let runner = Runner(writer)
    //Runner.run_multi tests reference targets
    //Runner.run_multi tests reference targets
    let tests = tests |> List.filter (fun x -> x?Name |> String.contains_insensitive "insert")
