...
namespace Imms
{
	...
	public partial class ImmVector<T> : AbstractSequential<T, ImmVector<T>> {
		...
		/// <param name="eq"></param>
		/// <returns></returns>
		public ImmVector<Tuple<T, TInner>> Join<TInner, TKey>(IEnumerable<TInner> inner, Func<T, TKey> oKeySelector,
		...
		/// <param name="eq">The equality comparer.</param>
		/// <returns></returns>
		public ImmVector<Tuple<TKey, IEnumerable<T>>> GroupBy<TKey>(Func<T, TKey> keySelector,
			IEqualityComparer<TKey> eq = null) {
			return GroupBy(keySelector, x => x, Tuple.Create, eq ?? EqualityComparer<TKey>.Default);
		}
	
		public ImmVector<TRElem> Cast<TRElem>() {
			return base.Cast<TRElem, ImmVector<TRElem>>(GetPrototype<TRElem>());
		}
