...
type ImmSetWrapper<'v when 'v : comparison>(Inner : ImmSet<'v>, Ordering : ImmSortedSet<'v>) = 
    ...
    let unwrap (vs : 'v seq) = 
        match vs with
        | :? ImmSetWrapper<'v> as wrapped -> wrapped.Inner :> _ seq
        | sq -> sq
    member val Inner = Inner
    member val Ordering = Ordering
    ...
    override x.All f = Inner |> ImmSet.forAll f
    override x.Any f = Inner |> ImmSet.exists f
    override x.Count f = Inner |> ImmSet.count f
    override x.Pick f = Inner |> ImmSet.pick f
    override x.Find f = Inner |> ImmSet.find f
    override x.Fold initial f = Inner |> ImmSet.fold initial f
    override x.Reduce f = Inner |> ImmSet.reduce f
