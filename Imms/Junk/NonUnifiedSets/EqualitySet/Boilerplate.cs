...
namespace Imm.Collections
{
	...
	{
		private ImmSet<TElem2> GetPrototype<TElem2>(IEqualityComparer<TElem2> ph)
		{
			return ImmSet<TElem2>.Empty(ph);
		}
		...
		/// <returns></returns>
		public ImmSet<TRElem> Select<TRElem>(Func<T, TRElem> selector, IEqualityComparer<TRElem> handler = null)
		{
		...
		/// <param name="handler">A new equality or comparison handler for constructing the resulting set.</param>
		/// <returns></returns>
		public ImmSet<TRElem> Select<TRElem>(Func<T, Option<TRElem>> selector, IEqualityComparer<TRElem> handler = null )
		{
			if (selector == null) throw Errors.Is_null;
		...
		/// <returns></returns>
		public ImmSet<TRElem> Cast<TRElem>(IEqualityComparer<TRElem> handler = null )
		{
			return Cast<TRElem, ImmSet<TRElem>>(GetPrototype<TRElem>(handler));
		}
		...
		/// <param name="handler">The handler.</param>
		/// <returns></returns>
		public ImmSet<TRElem> OfType<TRElem>(IEqualityComparer<TRElem> handler = null) 
		{
			return OfType<TRElem, ImmSet<TRElem>>(GetPrototype<TRElem>(handler));
		}
		...
		/// <param name="handler">A new equality or comparison handler for constructing the resulting set.</param>
		/// <returns></returns>
		public ImmSet<TRElem> Scan<TRElem>(TRElem initial, Func<TRElem, T, TRElem> accumulator, IEqualityComparer<TRElem> handler = null )
		{
